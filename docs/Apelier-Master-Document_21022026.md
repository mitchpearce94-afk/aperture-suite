...
**Version:** 4.3  
**Last Updated:** 21 February 2026 (AI pipeline DB/path fix, gallery upload UX overhaul, gallery settings cleanup, watermark system, password recall)  
...
**14 tables + 3 new tables + RLS policies per photographer. 18 migrations applied. Storage RLS simplified for uploads.**
...
| `galleries` | Photo collections | photographer_id, job_id, client_id, title, description, slug, access_type, download_permissions, brand_override, expires_at, status, view_count, password_hash, password_plain, delivered_at |
...
5. ~~**Client gallery watermarks:** CSS overlay watermark with photographer's logo on grid tiles and lightbox. Controlled by Settings > Gallery Settings "Show watermark on preview images" toggle~~ ✅ Done 21 Feb
...
### Session: 21 Feb 2026 — AI Pipeline Fix, Gallery Upload UX, Gallery Settings, Watermarks, Password Recall

**AI Pipeline DB Update + Storage Path Fix (2 bugs):**
- **Bug 1 — DB update method:** `orchestrator.py` called `config.update(table, filters, data)` but `config.update()` signature is `(table, id, data)`. All DB writes after AI processing silently failed — photos never got `web_key`, `thumb_key`, `edited_key` written
- **Bug 2 — Storage path collision:** Output phase uploaded edited files to `/uploads/` segment but originals also stored in `/originals/`. Edited files were overwriting originals. Fixed path to use `/edited/` segment
- Files: config.py, main.py, process.py, orchestrator.py

**Gallery Photo Upload UX Overhaul (3 issues fixed):**
- **Parallel uploads:** Rewrote sequential upload to 3-concurrent worker pool (`CONCURRENT_UPLOADS = 3`). 30 files now upload ~3x faster
- **Navigation protection:** `beforeunload` event warns if user tries to leave during upload. Red warning banner shown during active upload
- **Duplicate prevention:** On job selection, checks existing gallery photo count. Shows "X already uploaded" badge. Blue info banner explains new files append (no duplicates). Clears file list when switching jobs
- Additional: Package limit progress bar (green/amber/red), over-limit warning, error modal for failed uploads

**Progressive Hero Image:**
- Client gallery hero was using 400px `thumb_url` stretched to 70-80vh = pixelated
- Added `ProgressiveHeroImage` component: shows blurred thumbnail instantly (blur(24px) + scale), preloads 2048px `web_url` in background, crossfades to sharp version over 0.8s
- Radial vignette overlay for cinematic depth

**Gallery Photos API — Fallback URL Chain:**
- API only signed `web_key` and `thumb_key` — photos without AI processing had no URLs
- Added fallback chain: `web_url` cascades `web_key → edited_key → original_key`, `thumb_url` cascades `thumb_key → original_key`
- Photos now display at original resolution even before AI pipeline runs

**Gallery Settings Cleanup:**
- Removed per-gallery Access Type selector — now reads from photographer's global `gallery_default_access_type` in Settings → Gallery Settings
- Gallery settings panel always visible (removed toggle button), no more "Hide Settings" / "Close"
- Password field only shows when global default is 'password'

**Gallery Password — Save with Settings + Recall:**
- Removed standalone "Set" button — password now saves with the main "Save Settings" button
- Added `password_plain` column to galleries table (migration 20260221000001)
- On load, fetches saved plaintext password and displays masked (••••••••)
- Eye toggle icon to show/hide password — photographers can recall it weeks later
- API `get` action moved above the `!password` guard (was returning 400 for missing password)

**Photographer Logo Watermark (#39 DONE):**
- CSS overlay watermark on gallery grid tiles (1/3 width, max 120px, 35% opacity) and lightbox (1/4 width, max 200px, 30% opacity)
- Reads `gallery_default_watermark` from photographer settings via branding API
- Controlled by "Show watermark on preview images" toggle in Settings → Gallery Settings
- `pointer-events-none`, `select-none`, `draggable={false}` to discourage casual saving
- Only shows when logo is uploaded and watermark setting is enabled

**Hero Logo Size:**
- Bumped photographer logo on client gallery hero from `h-12` (48px) to `h-24 sm:h-32` (96-128px) with `drop-shadow-lg`

**Lightbox Overflow Fix:**
- Wrapping `div` for watermark caused image to overflow viewport
- Fixed with `max-h-[calc(100vh-8rem)]` on img element and `inline-flex` wrapper

**Files modified:** config.py, main.py, process.py, orchestrator.py, gallery/[slug]/page.tsx, photo-upload.tsx, gallery-detail.tsx, gallery-photos/route.ts, gallery-password/route.ts, gallery-branding/route.ts, 1 migration
