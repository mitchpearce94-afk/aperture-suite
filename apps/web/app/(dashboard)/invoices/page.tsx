  ...
  async function handleAutoGenerate() {
    ...
    if (requiresDeposit) {
      ...
      const finalInv = await createInvoice({
        ...
        notes: finalDue ? `Final payment due ${new Date(finalDue).toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })} (2 weeks before shoot).` : 'Final payment — remaining balance.',
      });
      ...
      const fullInv = await createInvoice({
        ...
        notes: fullDue ? `Payment due ${new Date(fullDue).toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })} (2 weeks before shoot).` : undefined,
      });
